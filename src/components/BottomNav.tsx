...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 z-50 w-full max-w-3xl -translate-x-1/2 border-t border-[var(--border)] bg-[var(--surface)]/95 backdrop-blur">
      <div className="mx-auto flex max-w-md items-end justify-around px-4 pb-2 pt-2">
        {TABS.slice(0, 2).map((tab) => (
          <TabLink key={tab.href} tab={tab} active={pathname === tab.href} />
        ))}

        <Link href="/report" className="flex flex-col items-center" aria-label="Report">
          <span
          ...
          <span className="mt-1 text-[10px] font-medium text-forest">Report</span>
        </Link>
